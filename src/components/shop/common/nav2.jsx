...
function Nav1({Nav, setNav}) {
  ...
  // this renders the navItems from the navlist
  const navTemp = (navItem) =>{
      return navItem.map((data)=>{
      return(
        <a 
          href={data.href}
          key={data.name}>
  ...
  // remove nav transition on window resize
  useEffect(() => {
